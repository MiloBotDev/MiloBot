package tk.milobot.commands.command.extensions;

import tk.milobot.commands.GuildPrefixManager;
import tk.milobot.commands.command.INewCommand;
...
public interface SlashCommand extends INewCommand {
    ...
    default void generateHelp(@NotNull SlashCommandEvent event) {
        String prefix;
    ...
    default @NotNull String getSlashCommandsArgumentsText(@NotNull String prefix) {
        ...
        if(getCommandData().getOptions().size() == 0) {
            argumentsText.append('`');
            return argumentsText.toString();
        }
        getCommandData().getOptions().forEach(optionData -> {
