...
public class BlackjackGame {
            ...
            try {
                ...
                if (playerBet < 0) {
                    ...
                    try (Connection con = DatabaseConnection.getConnection()) {
                        ...
                        tk.milobot.database.model.User user = userDao.getUserByDiscordId(con, event.getAuthor().getIdLong(), RowLockType.FOR_UPDATE);
    ...
    public static void newGame(@NotNull SlashCommandEvent event, int instanceDuration) {
        ...
        if (event.getOption("bet") == null) {
            ...
            try (Connection con = DatabaseConnection.getConnection()) {
                con.setAutoCommit(false);
                tk.milobot.database.model.User user = userDao.getUserByDiscordId(con, event.getUser().getIdLong(), RowLockType.FOR_UPDATE);
                int playerWallet = Objects.requireNonNull(user).getCurrency();
    ...
    private void updateWallet(Connection con, @Nullable BlackjackStates state) throws SQLException {
        tk.milobot.database.model.User user;
